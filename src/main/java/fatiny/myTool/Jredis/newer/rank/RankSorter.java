...
public class RankSorter extends RedisClient {
	...
	public RankSorter(String ip, int port, int maxTotal, int maxIdle,
		...
		ranks = new HashMap<String, RankDetail>();
	}
	...
	public void init(String rankName, int limit) {
		...
		try {
			...
			logger.debug("RankDetail:{}", rankDetail);
	...
	public void add(String rankName, ISorter sorter){
		...
		try {
			...
			if (rankDetail.isFull()) {
				//判断是否可以加入到排行榜
				if ((Double) sorter.getFirstOrder() > rankDetail.getMinScore()) {
					//缓存是否已经存在当前数据,则要减去1
					Double score = jedis.zscore(rankName, sorter.getSortId());
					if (score != null) {
						rankDetail.decrement();
					}
					jedis.zadd(rankName, sorter.getFirstOrder(), sorter.getSortId());
					rankDetail.increment();
				}
			}else{
				//排行榜没满, 直接插入
				jedis.zadd(rankName, sorter.getFirstOrder(), sorter.getSortId());
			...
			removeOverflow1(jedis, rankDetail, rankName);
	...
	private void removeIfoverflow(ShardedJedis jedis, RankDetail rankDetail, String rankName){
		if (rankDetail.isFlow()) {
			//溢出溢出部分数据下标起始
			long start = 0;
			long end = rankDetail.getSpillSize() - 1;

			long size = jedis.zremrangeByRank(rankName, start, end);
			//移除redis缓存数据
			rankDetail.addAndGet((-size));
			
			//logger.debug("removeIfoverflow, size:{}, cur:{}, redisSize:{}, end:{}", size, rankDetail.size(), jedis.zcard(rankName), end);
			//获取最后一名数据, 更新到排行信息里
			Tuple tuple = getLast(jedis, rankName);
			//重置最新数据
			if (tuple != null) 
	...
	private void removeOverflow1(ShardedJedis jedis, RankDetail rankDetail, String rankName){
		if (rankDetail.isFlow()) {
			long start = 0;
			long end = rankDetail.getSpillSize();
			Set<Tuple> set = jedis.zrevrangeWithScores(rankName, start, end);
	...
	private Tuple getLast(ShardedJedis jedis, String rankName){
		Tuple result = null;
		//Set<Tuple> set = jedis.zrangeWithScores(rankName, -1, -1);
...
//			}
			jedis = getConnect();
			logger.debug("rankDetail.size:{}, redis.size{}", rankDetail.size(), jedis.zcard(rankName));
	...
	public Double getScore(String rankName, String key) {
		...
		try {
			jedis = getConnect();
			return jedis.zscore(rankName, key);
		} catch (RuntimeException e) {
			returnExceptionConnection(jedis);
			logger.error("redis exception", e);
			throw e;
