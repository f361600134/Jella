...
import fatiny.myTool.juice.util.ClassPathScanner;
...
public class ParamAnalysis {
	...
	public static void analysis() throws Exception {
		...
		Set<Class<?>> classes = scan.getPackageAllClasses("fatiny.myTool.juice", true);
	...
	// TreeMap<String, ParamField> paramFieldMap =
	// PropertiesBase.getValue(propertyName);
	// // 基础类型和String类型
	...
	// ConvertUtils.convert(paramField.getValue(), clazzType));
	// }
	...
	// for (ParamField paramField : paramFieldMap.values()) {
	// // 值设置进去list
	// addMethod.invoke(list, ConvertUtils.convert(paramField.getValue(),
	// valueClazz));
	// }
	...
	// Class<?> keyClazz = Class.forName(keyActualType.getTypeName());
	// Class<?> valueClazz = Class.forName(valueActualType.getTypeName());
	// Method addMethod = clazzType.getDeclaredMethod("put", Object.class,
	// Object.class);
	// if (addMethod != null) {
	// for (ParamField paramField : paramFieldMap.values()) {
	// // 值设置进去list
	// addMethod.invoke(map, ConvertUtils.convert(paramField.getKeyIndex(),
	// keyClazz), ConvertUtils.convert(paramField.getValue(), valueClazz));
	// }
	// descriptor.getWriteMethod().invoke(obj, map);
	// }
	// } else {
	// logger.info("propertyName:{} 找不到类型, clazzType:{}", propertyName,
	// clazzType);
	// }
	// }
	// }
